...
vim.keymap.set('n', '<leader>pp', function() require('lazy').home() end, { desc = 'Plugins Status' })
...
require('lazy').setup({
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          -- Opens a popup that displays diagnostic messages about the line under your cursor
          map('gl', function()
            vim.diagnostic.open_float {
              format = function(diagnostic) return string.format('%s: %s', diagnostic.code, diagnostic.message) end,
            }
          ...
          map('<Leader>lll', vim.diagnostic.setloclist, 'Add buffer Diagnostics to the [L]ocation [L]ist')
          map('<Leader>llq', vim.diagnostic.setqflist, 'Add all Diagnostics to the [Q]uickfix [L]ist')

          vim.keymap.set('i', '<C-g>', vim.lsp.buf.signature_help, { buffer = event.buf, desc = 'LSP: Show Signature Help' })
  ...
  { -- Colorscheme
    ...
    priority = 1000,
    init = function(_) vim.cmd.colorscheme 'nordic' end,
  },
